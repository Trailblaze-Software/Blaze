...
[**ClassList**](annotated.md) **>** [**DemLayer**](classDemLayer.md)
...
| Type | Name |
| ---: | :--- |
|   | [**DemLayer**](#function-demlayer) (const fs::path & dem\_path, [**AsyncProgressTracker**](classAsyncProgressTracker.md) progress\_tracker, const std::optional&lt; fs::path &gt; & texture\_path=std::nullopt, const std::string & target\_crs\_wkt={}, bool flat\_cells=false) <br> |
...
```C++
inline virtual Extent3D DemLayer::extent () override const
```
...
```C++
inline virtual std::string DemLayer::native_projection () override const
```
...
```C++
inline virtual std::string DemLayer::projection () override const
```
...
```C++
inline const AsyncRasterData & DemLayer::raster () const
```
...
```C++
inline AsyncRasterData & DemLayer::raster () 
```
