

# Struct ContourConfig



[**ClassList**](annotated.md) **>** [**ContourConfig**](structContourConfig.md)


























## Public Attributes

| Type | Name |
| ---: | :--- |
|  ColorVariant | [**color**](#variable-color)  <br> |
|  double | [**interval**](#variable-interval)  <br> |
|  unsigned int | [**min\_points**](#variable-min_points)  <br> |
|  double | [**width**](#variable-width)  <br> |












































## Public Attributes Documentation




### variable color 

```C++
ColorVariant ContourConfig::color;
```




<hr>



### variable interval 

```C++
double ContourConfig::interval;
```




<hr>



### variable min\_points 

```C++
unsigned int ContourConfig::min_points;
```




<hr>



### variable width 

```C++
double ContourConfig::width;
```




<hr>

------------------------------
The documentation for this class was generated from the following file `/home/runner/work/Blaze/Blaze/src/lib/config_input/config_input.hpp`

