

# Struct UserCrsParseResult



[**ClassList**](annotated.md) **>** [**UserCrsParseResult**](structUserCrsParseResult.md)


























## Public Attributes

| Type | Name |
| ---: | :--- |
|  std::string | [**error**](#variable-error)  <br> |
|  bool | [**ok**](#variable-ok)  <br> |
|  std::string | [**wkt**](#variable-wkt)  <br> |












































## Public Attributes Documentation




### variable error 

```C++
std::string UserCrsParseResult::error;
```




<hr>



### variable ok 

```C++
bool UserCrsParseResult::ok;
```




<hr>



### variable wkt 

```C++
std::string UserCrsParseResult::wkt;
```




<hr>

------------------------------
The documentation for this class was generated from the following file `/home/runner/work/Blaze/Blaze/src/lib/io/crs.hpp`

