

# Struct nlohmann::adl\_serializer&lt; WaterConfigs &gt;

**template &lt;&gt;**



[**ClassList**](annotated.md) **>** [**nlohmann**](namespacenlohmann.md) **>** [**adl\_serializer&lt; WaterConfigs &gt;**](structnlohmann_1_1adl__serializer_3_01WaterConfigs_01_4.md)












































## Public Static Functions

| Type | Name |
| ---: | :--- |
|  [**WaterConfigs**](structWaterConfigs.md) | [**from\_json**](#function-from_json) (const json & j) <br> |
|  void | [**to\_json**](#function-to_json) (json & j, [**WaterConfigs**](structWaterConfigs.md) cc) <br> |


























## Public Static Functions Documentation




### function from\_json 

```C++
static inline WaterConfigs nlohmann::adl_serializer< WaterConfigs >::from_json (
    const json & j
) 
```




<hr>



### function to\_json 

```C++
static inline void nlohmann::adl_serializer< WaterConfigs >::to_json (
    json & j,
    WaterConfigs cc
) 
```




<hr>

------------------------------
The documentation for this class was generated from the following file `/home/runner/work/Blaze/Blaze/src/lib/config_input/config_input.cpp`

