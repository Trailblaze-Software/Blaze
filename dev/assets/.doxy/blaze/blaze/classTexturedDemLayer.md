...
[**ClassList**](annotated.md) **>** [**TexturedDemLayer**](classTexturedDemLayer.md)
...
| Type | Name |
| ---: | :--- |
|   | [**TexturedDemLayer**](#function-textureddemlayer) (const fs::path & dem\_path, const fs::path & texture\_path, [**AsyncProgressTracker**](classAsyncProgressTracker.md) progress\_tracker, const std::string & target\_crs\_wkt={}) <br> |
...
### function TexturedDemLayer 

```C++
inline TexturedDemLayer::TexturedDemLayer (
...
```C++
inline virtual Extent3D TexturedDemLayer::extent () override const
```
...
```C++
inline virtual std::string TexturedDemLayer::native_projection () override const
```
...
```C++
inline virtual std::string TexturedDemLayer::projection () override const
```
...
```C++
inline const AsyncRasterData & TexturedDemLayer::raster () const
```
...
```C++
inline AsyncRasterData & TexturedDemLayer::raster () 
```
