

# Struct GroundConfig



[**ClassList**](annotated.md) **>** [**GroundConfig**](structGroundConfig.md)


























## Public Attributes

| Type | Name |
| ---: | :--- |
|  [**int**](classCoordinate2D.md) | [**max\_ground\_intensity**](#variable-max_ground_intensity)   = `1000`<br> |
|  [**int**](classCoordinate2D.md) | [**min\_ground\_intensity**](#variable-min_ground_intensity)   = `100`<br> |












































## Public Attributes Documentation




### variable max\_ground\_intensity 

```C++
int GroundConfig::max_ground_intensity;
```




<hr>



### variable min\_ground\_intensity 

```C++
int GroundConfig::min_ground_intensity;
```




<hr>

------------------------------
The documentation for this class was generated from the following file `/home/runner/work/Blaze/Blaze/src/lib/config_input/config_input.hpp`

