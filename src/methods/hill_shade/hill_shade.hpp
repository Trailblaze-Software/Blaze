...
GeoGrid<double> hill_shade(const GeoGrid<T>& grid, double azimuth=315, double altitude=45, bool multidirectional=true) {
  ...
  GeoGrid<double> result(grid.width(), grid.height(), GeoTransform(grid.transform()), GeoProjection(grid.projection()));
  for (size_t i = 1; i < grid.height()-1; i++) {
    for (size_t j = 1; j < grid.width()-1; j++) {
      ...
      if (!multidirectional) {
        hill_shade_val = (cos(DEG2RAD(altitude)) * cos(slope) + sin(DEG2RAD(altitude)) * sin(slope) * cos(DEG2RAD(azimuth) - aspect));
      } else {
        // Multidirectional hillshade https://pubs.usgs.gov/of/1992/of92-422/of92-422.pdf
        for (double az : std::array<double, 4>{azimuth-67.5, azimuth-22.5, azimuth+22.5, azimuth+67.5}) {
          hill_shade_val += SQ(sin(aspect-DEG2RAD(az))) * (cos(DEG2RAD(altitude)) * cos(slope) + sin(DEG2RAD(altitude)) * sin(slope) * cos(DEG2RAD(az) - aspect));
        }
      }
