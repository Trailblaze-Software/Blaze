...
public:
  ...
  bool orthogonal_to(Direction2D other) const {
    return ((m_dir == UP || m_dir == DOWN) && (other.m_dir == LEFT || other.m_dir == RIGHT)) || ((m_dir == LEFT || m_dir == RIGHT) && (other.m_dir == UP || other.m_dir == DOWN));
  }

  Direction2D opposite() const {
    switch (m_dir) {
      case UP: return Direction2D(DOWN);
      case DOWN: return Direction2D(UP);
      case LEFT: return Direction2D(RIGHT);
      case RIGHT: return Direction2D(LEFT);
    }
  }
...
public:
  ...
  Coordinate2D operator+(Direction2D dir) const {
    switch (dir) {
      case Direction2D::UP: return Coordinate2D(x(), y() - 1);
      case Direction2D::DOWN: return Coordinate2D(x(), y() + 1);
      case Direction2D::LEFT: return Coordinate2D(x() - 1, y());
      case Direction2D::RIGHT: return Coordinate2D(x() + 1, y());
    }
  }
};
...
std::array<Coordinate2D<T>, 4> orthogonal_neighbours(const Coordinate2D<T>& coord) {
  return {
    Coordinate2D<T>(coord.x() - 1, coord.y()),
    Coordinate2D<T>(coord.x() + 1, coord.y()),
    Coordinate2D<T>(coord.x(), coord.y() - 1),
    Coordinate2D<T>(coord.x(), coord.y() + 1)
  };
}
...
public:
  LineCoord2DCrossing(T x, T y, Direction2D dir, Direction2D crossing_dir) : LineCoord2D<T>(x, y, dir), m_crossing_dir(crossing_dir) {}
  LineCoord2DCrossing(const LineCoord2D<T>& line_coord, Direction2D crossing_dir) : LineCoord2D<T>(line_coord), m_crossing_dir(crossing_dir) {}

  Direction2D crossing_dir() const { return m_crossing_dir; }

  LineCoord2DCrossing flip() const {
    return LineCoord2DCrossing({LineCoord2D<T>::end(), LineCoord2D<T>::dir().opposite()}, m_crossing_dir);
  }
  ...
  std::vector<LineCoord2DCrossing> next_points() const {
    ...
    if (LineCoord2D<T>::dir().orthogonal_to(crossing_dir())){
      ...
      for (Direction2D dir : {Direction2D::UP, Direction2D::DOWN, Direction2D::LEFT, Direction2D::RIGHT}) {
