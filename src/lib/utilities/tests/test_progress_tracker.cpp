...
TEST(ProgressTracker, ProgressTracker) {
  ...
  EXPECT_EQ(tracker1.m_proportion, 0.0);

  tracker1.set_proportion(0.1);
  EXPECT_EQ(tracker1.m_proportion, 0.1);

  {
    ProgressTracker trackerA(tracker1.subtracker(0.2, 0.6));
    EXPECT_EQ(tracker1.m_proportion, 0.2);

    trackerA.set_proportion(0.25);
    EXPECT_DOUBLE_EQ(tracker1.m_proportion, 0.3);
  }

  EXPECT_DOUBLE_EQ(tracker1.m_proportion, 0.6);

  {
    ProgressTracker trackerB = tracker1.subtracker(0.7, 0.9);

    trackerB.set_proportion(0.5);
    EXPECT_DOUBLE_EQ(tracker1.m_proportion, 0.8);
  }

  {
    ...
    EXPECT_DOUBLE_EQ(tracker1.m_proportion, 0.9);
  }
