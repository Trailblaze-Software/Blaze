...
public:
  ...
  friend std::ostream& operator<<(std::ostream& os, const Grid& grid) {
    os << "Grid<" << typeid(T).name() << ">(" << grid.width() << ", " << grid.height() << ")" << std::endl;
...
public:
  GeoGrid(size_t width, size_t height, GeoTransform&& transform, GeoProjection&& projection) : Grid<T>(width, height), m_transform(transform), m_projection(projection) {}
  ...
  template <typename U>
    GeoGrid<T> FromPoints(const GeoGrid<U>& grid) {
      return GeoGrid<T>(grid.width() - 1, grid.height() - 1, GeoTransform(grid.transform().x() + grid.transform().dx() / 2, grid.transform().y() + grid.transform().dy() / 2, grid.transform().dx(), grid.transform().dy()), grid.projection());
    }
};
...
class GridGraph {
  ...
  public:
  template <typename U>
  explicit GridGraph(const Grid<U>& grid) : m_horizontal(grid.width()-1, grid.height()), m_vertical(grid.width(), grid.height()-1) {}
